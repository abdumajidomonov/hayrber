...
class ProductBarcodeWizardFinal(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        scan_picking_id = ctx.get('scan_picking_id', 0)
        scan_quantities = ctx.get('scan_quantities', {})

        if scan_picking_id:
            res['scan_picking_id'] = scan_picking_id
        ...
        if active_ids and active_model:
            ...
            lines = [
                (0, 0, {
                    'product_id': p.id,
                    'qty': scan_quantities.get(p.id, 1),
                })
                for p in products if p.exists()
            ]
            if lines:
                res['line_ids'] = lines
        return res

    # Scan wizarddan kelgan picking ID (receipt ochish uchun)
    scan_picking_id = fields.Integer(default=0)
    ...
    line_ids = fields.One2many(
        'product.barcode.wizard.line.final', 'wizard_id', string='Mahsulotlar'
    )
    preview_html = fields.Html(compute='_compute_preview_html')

    @api.depends(
        'label_width', 'label_height', 'border_thickness', 'border_color',
        'line_color', 'margin', 'padding_top', 'padding_bottom',
        'padding_left', 'padding_right', 'font_size_name', 'name_line_height',
        'barcode_height', 'font_size_barcode', 'line_ids'
    )
    def _compute_preview_html(self):
        for rec in self:
            sample_name = (
                rec.line_ids[0].product_id.name if rec.line_ids else "MAHSULOT NOMI"
            )
            sample_bc = (
                rec.line_ids[0].product_id.barcode
                if rec.line_ids and rec.line_ids[0].product_id.barcode
                else "123456789012"
            )
            html = f"""
                ...
                <div style="width:{rec.label_width}mm; height:{rec.label_height}mm; padding:{rec.margin}mm; box-sizing:border-box; display:inline-block; font-size:0; line-height:0;">
                    <div style="width:100%; height:100%; background:white; padding:{rec.padding_top}mm {rec.padding_right}mm {rec.padding_bottom}mm {rec.padding_left}mm; box-sizing:border-box; overflow:hidden;">
                        <table style="width:100%; height:100%; border-collapse:collapse; table-layout:fixed; font-size:10pt; line-height:normal;">
                            <tr style="height:25%;"><td style="vertical-align:middle; text-align:center;">
                            ...
                            <tr style="height:1pt;"><td style="border-top:0.5pt solid {rec.line_color}; padding:0;"></td></tr>
                            <tr style="height:45%;"><td style="vertical-align:middle; text-align:center; padding:2px 0;">
    ...
    def action_print(self):
        # Scan wizarddan kelgan bo'lsa: PDF yuklanadi + picking formi ochiladi
        if self.scan_picking_id:
            return {
                'type': 'ir.actions.act_url',
                'url': (
                    f'/product_barcode_auto_gen/print_wizard_and_open'
                    f'?wizard_id={self.id}&picking_id={self.scan_picking_id}'
                ),
                'target': 'self',
            }
        # Odatiy holat: faqat PDF yuklanadi
        return self.env.ref(
            'product_barcode_auto_gen.action_report_product_barcode_multi_final'
        ).report_action(self)
